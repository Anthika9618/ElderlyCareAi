...
def wait_for_ok_hand_gesture (stream_url, timeout = 10) :
    ...
    with mp_hands.Hands(
        ...
        while time.time() - start_time < timeout :
            ret, frame = cap.read ()
            if not ret :
                print("ไม่สามารถอ่านถาพจากSteamได้")
                break
            ...
            if result.multi_hand_landmarks :
                for hand_landmarks in result.multi_hand_landmarks :
                    ...
                    if index_tip.y < wrist.y : 
                        print("ยกมือขึ้น ถือว่าปลอดภัยแล้ว")
                        cap.release()
                        return True 

            cv2.imshow('Hand Gesture Check (Streaming)', frame)
...
def fall_response_process(stream_url) : 
    print("ตรวจพบการล้ม รอการยืนยันการล้ม...")

    gesture_confirmed = wait_for_ok_hand_gesture (stream_url, timeout = 10)

    if gesture_confirmed :
        print("ยืนยันความปลอดภัย - ไม่ต้องมีการแจ้งเตือน")
    else : 
        print("ไม่พบการยืนยัน - กำลังส่งแจ้งเตือนฉุกเฉิน")
        send_emergency_alert()


def send_emergency_alert():
    print("[ALERT] ⚠️ แจ้งเตือนเหตุฉุกเฉินส่งไปยังเจ้าหน้าที่แล้ว!")
    ...
    while not stop_event.is_set():
        ...
        while not stop_event.is_set():
            ...
            if result is True:
                ...
                # เรียกตรวจสอบ gesture OK เพื่อยืนยัน
                threading.Thread(
                    target=fall_response_process, 
                    args=(source,),  # ใช้ stream_url หรือ source
                    daemon=True
                ).start()
