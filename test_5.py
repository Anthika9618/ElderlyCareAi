...
def is_standing_pose(keypoints, height_threshold=0.15, angle_threshold=40):
    """
    ตรวจสอบว่าท่ายืนหรือไม่ โดยดูจาก
    - สะโพกต้องสูงกว่าข้อเท้าพอสมควร (height_threshold)
    - มุมลำตัวต้องไม่เกิน angle_threshold (ใกล้ตั้งตรง)
    """
    try:
        hip_y = keypoints[23*3 + 1]
        ankle_y = keypoints[27*3 + 1]
...
# ===== Core Detection =====
def detect_fall(sequence, results, index):
    global last_log_time, last_person_detected, fall_counters, last_debug_log_time
    ...
    if current_time - last_debug_log_time[index] > 5:
...
def capture_stream(index, source):
    ...
    while True:
        ...
        while True:
            ...
            result = detect_fall(sequence_list[index], results, index)
            if result is True:
